...
evaluatesetting <- function(iter) {
  ...
  for(i in seq(along=block)) {
    if(length(block[[i]]) <= 2500) {
      ...
      inds <- which(colnames(Xtrain) %in% colnames( Xtrain_snz[,order(varimp,decreasing = T)[1:2500]]))
...
##reduce memory
  ...
  save(res, file=paste("./VarselCompStudy/Additional_file/Results/Results1/res", iter, ".Rda", sep=""))
